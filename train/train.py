...
from model.language_model.llaga_llama import DescriptionLlagaLlamaForCausalLM, LlagaLlamaForCausalLM_fix
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        ...
        keys_to_match.extend(['gat_w', 'gat_a'])
...
class LazySupervisedGraphDataset(Dataset):
    ...
    def __init__(self,
        ...
        for d, dataset in enumerate(self.use_dataset):
            ...
            if "arxiv" in dataset:
                data_path = autodl_prefix + "ogbn-arxiv/processed_data.pt"
            elif "products"  in dataset:
                data_path = autodl_prefix + "ogbn-products/processed_data.pt"
            ...
            if data_args.template == "ND":
                ...
                self.structure_emb = torch.load(autodl_prefix+
                    f"laplacian_{data_args.use_hop}_{data_args.sample_neighbor_size}.pt")
                        ...
                        with open(data_path, 'r') as file:
                            for line in file:
                                ...
                                if dataset in ["arxiv", "cora", "pubmed"]:
                                    ...
                                    '''
                                    if title == "":
                                        assistant_prompt = f"This is a paper."
                                        count +=1 
                                        continue
                                    else:
                                        assistant_prompt = f"This is a paper about {title}."
                                    ...
                                    '''
                                    if desc == "":
                                        assistant_prompt = f"This is an amazon product."
                                        continue
                                    else:
                                        assistant_prompt = f"This is an amazon product, it can be described as {desc}"
                                else:
                                    raise ValueError
                                l["conversations"] = [{'from': 'human', 'value': user_prompt},
                                                      {'from': 'gpt', 'value': assistant_prompt}]
                                task_list_data_dict.append(l)
                        print("无标题信息的数量为：", count)
                        print("总共数量为：", all_count)
                elif task == "nda":
                    if data_args.template == "HO":
                        data_path = os.path.join(data_dir,
                                                 f"sampled_2_10_train.jsonl")
                            ...
                            for line in file:
                                ...
                                if dataset in ["arxiv", "cora", "pubmed"]:
                                    ...
                                    if ab == "":
                                        ...
                                        assistant_prompt = f"It is about {ab}."
                                elif dataset == "products":
                                    desc = data.raw_texts[id]
                                    # if desc == "":
                                    #     assistant_prompt = f"This is an amazon product which can be categorized as {label}."
                                    ...
                                    if desc == "":
                                        ...
                                        assistant_prompt = f"It is about {desc}"
                                else:
                                    raise ValueError
                            ...
                            for line in file:
                                ...
                                if dataset in ["arxiv", "cora", "pubmed"]:
                                    ...
                                    if title == "" and ab == "":
                                        ...
                                        assistant_prompt = f"This is a paper, its title is {title}."
                                    elif ab == "":
                                        assistant_prompt = f"This is a paper, its abstract is {ab}."
                                    else:
                                        assistant_prompt = f"This is a paper, its title is {title}, its abstract is {ab}."
                                elif dataset == "products":
                                    desc = data.raw_texts[id]
                                    # if desc == "":
                                    #     assistant_prompt = f"This is an amazon product which can be categorized as {label}."
                        ...
                        with open(data_path, 'r') as file:
                            for line in file:
                                ...
                                if dataset == "cora":
                                    ...
                                    l["conversations"][1]['value'] = label_dict[l["conversations"][1]['value']]
                                task_list_data_dict.append(l)
                    else:
                        raise ValueError
                else:
...
class LazySupervisedGraphDataset_GNN(Dataset):
    """Dataset for supervised fine-tuning."""

    def __init__(self,
                 tokenizer: transformers.PreTrainedTokenizer,
                 data_args: DataArguments):
        super(LazySupervisedGraphDataset_GNN, self).__init__()
        self.use_dataset = data_args.use_dataset.split('-')
        self.use_hop = data_args.use_hop
        self.template = data_args.template
        ...
        for d, dataset in enumerate(self.use_dataset):
            ...
            #     raise ValueError
            pretrained_emb = self.load_pretrain_embedding_graph(data_dir, data_args.pretrained_embedding_type)
            self.induced_graph[dataset] = self.load_induced_graph(data_dir)
            self.pretrained_embs[dataset] = pretrained_emb
            ...
            for task in self.use_task:
                ...
                if task == "nc":
                    ...
                    # else:
                    #     raise ValueError
                    human_convers = {"from":"human"}
                    human_convers["value"] = f"Given a node-centered graph: {DEFAULT_GRAPH_TOKEN}, each node represents a paper, we need to classify the center node into 7 classes: Case_Based, Genetic_Algorithms, Neural_Networks, Probabilistic_Methods, Reinforcement_Learning, Rule_Learning, Theory, please tell me which class the center node belongs to?"
                    for id in node_ids:
                        l = {"dataset":dataset}
                        #construct conversations
                        conversation_list = [human_convers]
                        gpt_convers = {"from":"gpt"}
                        gpt_convers["value"] = label_texts[y[id]]
                        conversation_list.append(gpt_convers)
                        l["conversations"] = conversation_list
                        l["id"] = id
                        task_list_data_dict.append(l)
    ...
    def load_induced_graph(self, data_dir):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 1:
            ...
            data_dict['induced_graph'] = self.induced_graph[self.list_data_dict[i]["dataset"]][id]
            # induced_graph = self.induced_graph[self.list_data_dict[i]["dataset"]][i]
            # data_dict['x'] = induced_graph['x']
            # data_dict['edge_index'] = induced_graph['edge_index']
        return data_dict
    
class InterpretationSupervisedGraphDataset(Dataset):
    """Dataset for supervised fine-tuning."""

    def __init__(self,
                 tokenizer: transformers.PreTrainedTokenizer,
                 data_args: DataArguments):
        super(InterpretationSupervisedGraphDataset, self).__init__()
        self.use_dataset = data_args.use_dataset.split('-')
        self.use_hop = data_args.use_hop
        self.template = data_args.template
        self.datas={}
                    ...
                    if os.path.exists(data_path):
                        with open(data_path, 'r') as file:
                            for line in file:
                                ...
                                l["conversations_desc"] = [{'from': 'human', 'value': user_prompt},
                                                      {'from': 'gpt', 'value': assistant_prompt}]
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        sources_interpretation = copy.deepcopy([e["conversations_desc"] for e in sources])
        data_dict = preprocess(
            sources_interpretation,
            self.tokenizer,
            has_graph=('graph' in self.list_data_dict[i]))
        if isinstance(i, int):
            data_dict = dict(input_ids_desc=data_dict["input_ids"][0],
                             labels_desc=data_dict['labels'][0])
            
        sources = copy.deepcopy([e["conversations"] for e in sources])
        data_dict_tem = preprocess(
            sources,
            self.tokenizer,
            has_graph=('graph' in self.list_data_dict[i]))
        if isinstance(i, int):
            # data_dict = dict(input_ids=data_dict["input_ids"][0],
            #                  labels=data_dict["labels"][0])
            data_dict["input_ids"] = data_dict_tem["input_ids"][0]
            data_dict["labels"] = data_dict_tem["labels"][0]
...
@dataclass
class DataCollatorForSupervisedDataset_GNN(object):
    """Collate examples for supervised fine-tuning."""

    tokenizer: transformers.PreTrainedTokenizer

    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        input_ids, labels, induced_graph = tuple([instance[key] for instance in instances]
                                  for key in ("input_ids", "labels", "induced_graph"))
        input_ids = torch.nn.utils.rnn.pad_sequence(
            input_ids,
            batch_first=True,
            padding_value=self.tokenizer.pad_token_id)
        ...
        #     batch['graph'] = torch.cat(graph, dim=0)
        #     batch['graph_emb'] = torch.cat(graph_emb, dim=0)
        #induced_graph = [instance['induced_graph'] for instance in instances]
        graph = Batch.from_data_list(induced_graph)
        # batch['x'] = graph.x
        # batch['edge_index']=graph.edge_index
        # batch['batch'] = graph.batch
        batch['induced_graph'] = graph
        return batch
    
@dataclass
class DataCollatorForSupervisedDatasetDescription(object):
    """Collate examples for supervised fine-tuning."""

    tokenizer: transformers.PreTrainedTokenizer

    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        input_ids_desc, input_ids, labels_desc, labels = tuple([instance[key] for instance in instances]
                                  for key in ("input_ids_desc", "input_ids", "labels_desc", "labels"))
        input_ids_desc = torch.nn.utils.rnn.pad_sequence(
            input_ids_desc,
            batch_first=True,
            padding_value=self.tokenizer.pad_token_id)
        input_ids = torch.nn.utils.rnn.pad_sequence(
            ...
            batch_first=True,
            padding_value=self.tokenizer.pad_token_id)
        labels_desc = torch.nn.utils.rnn.pad_sequence(labels_desc,
                                                 batch_first=True,
                                                 padding_value=IGNORE_INDEX)
        labels = torch.nn.utils.rnn.pad_sequence(labels,
                                                 batch_first=True,
        ...
        input_ids_desc = input_ids_desc[:, :self.tokenizer.model_max_length]
        input_ids = input_ids[:, :self.tokenizer.model_max_length]
        labels_desc = labels_desc[:, :self.tokenizer.model_max_length]
        labels = labels[:, :self.tokenizer.model_max_length]
        batch = dict(
            input_ids_desc=input_ids_desc,
            input_ids=input_ids,
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    train_dataset = LazySupervisedGraphDataset_GNN(tokenizer=tokenizer,
                                data_args=data_args)
    data_collator = DataCollatorForSupervisedDataset_GNN(tokenizer=tokenizer)
...
def _train():
    ...
    if 'mpt' in model_args.model_name_or_path:
        ...
        # model = DescriptionLlagaLlamaForCausalLM.from_pretrained(
        #     model_args.model_name_or_path,
        #     cache_dir=training_args.cache_dir,
        #     **bnb_model_from_pretrained_args
        # )
    ...
    if model_args.tune_mm_mlp_adapter:
        ...
        for p in model.get_model().gat_w.parameters():
            p.requires_grad = True
        for p in model.get_model().gat_a.parameters():
            p.requires_grad = True
        for p in model.get_model().gnn.parameters():
            p.requires_grad = True
    ...
    if training_args.freeze_mm_mlp_adapter:
        ...
        for p in model.get_model().gat_w.paramaters():
            p.requires_grad = False
        for p in model.get_model().gat_a.paramaters():
            p.requires_grad = False
        for p in model.get_model().gnn.parameters():
            p.requires_grad = True
    ...
    if training_args.bits in [4, 8]:
        ...
        model.get_model().gat_w.to(dtype=compute_dtype, device=training_args.device)
        model.get_model().gat_a.to(dtype=compute_dtype, device=training_args.device)
        model.get_model().gnn.to(dtype=compute_dtype, device=training_args.device)
