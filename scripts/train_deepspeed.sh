...
if [ ${model} = "vicuna" ]; then
  ...
  model_base=/root/autodl-tmp/lht/huggingface_model/lmsys/vicuna-7b-v1.5-16k
  mode="v1"
  ...
  model_base=/root/autodl-tmp/lht/llama-2-7b-chat-hf
  mode="llaga_llama_2"
...
wandb offline
echo deepspeed  --include localhost:3,4 --master_port 61000  train/train_mem.py \
...
#--pretrain_mm_mlp_adapter /root/lht/LLaGA/checkpoints/pubmed-arxiv/llaga-vicuna-7b-simteg-2-10-linear-projector_nd/mm_projector.bin \
...
--mm_use_graph_patch_token False \
--bf16 True \
--output_dir  ./checkpoints/${dataset}/${prefix}_${task}_selecttmp \
--num_train_epochs 1 \
--per_device_train_batch_size ${bs} \
--per_device_eval_batch_size 4 \
--gradient_accumulation_steps 1 \
