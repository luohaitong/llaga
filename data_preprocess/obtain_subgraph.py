...
def induced_graphs_nodes(data, pretrained_emb, dataname: str = None, smallest_size=100, largest_size=300):
    """
    ...
    induced_graph_list = []
    node_list = torch.arange(data.x.shape[0])
    iteration = 0

    for node in torch.flatten(node_list):
        ...
        if len(subset) < smallest_size:
            ...
            pos_nodes = torch.argwhere(data.y == int(label[node]))
        ...
        x = pretrained_emb[subset]
        y = label[subset]
        induced_graph = Data(x=x, edge_index=sub_edge_index, y=y)
        induced_graph_list.append(induced_graph)
        print('graph size {} at {:.2f}%...'.format(induced_graph["x"].shape[0], iteration * 100.0 / node_list.shape[0]))

    induced_graph_dic_list['pos'] = induced_graph_list


    pk.dump(induced_graph_dic_list,
            open('/root/autodl-tmp/lht/LLaGA/{}/induced_graphs'.format(dataname), 'bw'))
...
if __name__ == '__main__':
    dataname = 'cora'  # 'CiteSeer'  # 'PubMed' 'Cora'
    #
    ...
    induced_graphs_nodes(data, pretrained_emb, dataname=dataname, smallest_size=100, largest_size=111)
